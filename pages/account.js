...
export default class extends Page {
  ...
  async onSubmit(e) {
    ...
    const encodedForm = Object.keys(formData).map((key) => `${encodeURIComponent(key)  }=${  encodeURIComponent(formData[key])}`).join(`&`)
...
export class LinkAccounts extends React.Component {
  render() {
    return (
      <React.Fragment>
        {
          Object.keys(this.props.linkedAccounts).map((provider, i) => <LinkAccount key={i} provider={provider} session={this.props.session} linked={this.props.linkedAccounts[provider]}/>)
        }
      </React.Fragment>
    )
  }
